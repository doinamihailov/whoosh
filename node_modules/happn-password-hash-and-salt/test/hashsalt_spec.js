...
var splitHash = function (hash) {
    ...
    if (opt.length < 4)
        throw new Error('Hash expected to have four parts or more')
    return {
        algorithm: opt[0],
        iterations: opt[1],
        hash: opt[2],
...
describe('Password hash and salt', function () {
    ...
    describe('Hash verification', function () {
        ...
        it('should verify correct passwords, with a defined hash', function (done) {
            password('secret').hash(function (error1, key1) {
        ...
        it('should be backward compatible with old password hash formats', function (done) {
            ...
            password('secret').hash(function (error1, key1) {
                ...
                var oldFormat = key1.replace('$sha1', '');

                password('secret').verifyAgainst(oldFormat, function (error2, validated) {
                    expect(error2).to.not.exist;
                    expect(validated).to.equal(true);
                    done();
                });
        ...
        it('should verify against stored hash algorithm and iterations', function (done) {
            password('secret', 1000).hash(function (error1, key1) {
                var split = splitHash(key1);
                expect(split.algorithm).to.equal('pbkdf2');
                expect(split.iterations).to.equal('1000');
                password('secret').verifyAgainst(key1, function (error3, validated) {
                    expect(error3).to.not.exist;
                    expect(validated).to.equal(true);
                    done();
                });
